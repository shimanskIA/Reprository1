...
public class MainFrame extends JFrame {
    ...
    private JTextField memFieldX;
    private JTextField memFieldY;
    private JTextField memFieldZ;
    ...
    public MainFrame() throws IOException {
        ...
        JLabel memResultX = new JLabel("MEM X");
        memFieldX = new JTextField("0", 10);
        memFieldX.setMaximumSize(memFieldX.getPreferredSize());
        JLabel memResultY = new JLabel("MEM Y");
        memFieldY = new JTextField("0", 10);
        memFieldY.setMaximumSize(memFieldY.getPreferredSize());
        JLabel memResultZ = new JLabel("MEM Z");
        memFieldZ = new JTextField("0", 10);
        memFieldZ.setMaximumSize(memFieldZ.getPreferredSize());
        Box hboxMemVariables = Box.createHorizontalBox();
        hboxMemVariables.setBorder(BorderFactory.createLineBorder(Color.BLUE));
        hboxMemVariables.add(Box.createHorizontalGlue());
        hboxMemVariables.add(memResultX);
        hboxMemVariables.add(Box.createHorizontalStrut(10));
        hboxMemVariables.add(memFieldX);
        hboxMemVariables.add(Box.createHorizontalStrut(100));
        hboxMemVariables.add(memResultY);
        hboxMemVariables.add(Box.createHorizontalStrut(10));
        hboxMemVariables.add(memFieldY);
        hboxMemVariables.add(Box.createHorizontalStrut(100));
        hboxMemVariables.add(memResultZ);
        hboxMemVariables.add(Box.createHorizontalStrut(10));
        hboxMemVariables.add(memFieldZ);
        hboxMemVariables.add(Box.createHorizontalGlue());
        ...
        Mem.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent ev)
            {
                if (a == 1)
                {
                    ...
                    memFieldX.setText(mem1.toString());
                }
                if (a == 2)
                {
                    mem2 = Double.parseDouble(textFieldY.getText());
                    memFieldY.setText(mem2.toString());
                }
                if (a == 3)
                {
                    mem3 = Double.parseDouble(textFieldZ.getText());
                    memFieldZ.setText(mem3.toString());
                }
            }
        });
